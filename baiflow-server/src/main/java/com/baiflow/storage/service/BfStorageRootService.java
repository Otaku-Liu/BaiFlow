 ...
 */
public interface BfStorageRootService extends IService<BfStorageRoot> {
     ...
     */
    BfStorageRoot getByIdOrThrow(String id);

    /**
     * 将配置的 {@code rootPath} 解析为绝对路径并规范化。这是所有文件操作必须遵循的参考锚点。
     */
    Path resolveRootPath(BfStorageRoot root);

    /**
     * 核心路径穿越防护：验证 {@code resolved} 路径是否在存储根目录的绝对路径范围内。
     * 如果不在范围内则抛出 FILE_OPERATION_FAILED 业务异常。
     */
    void verifyPathInRoot(BfStorageRoot root, Path resolved);
