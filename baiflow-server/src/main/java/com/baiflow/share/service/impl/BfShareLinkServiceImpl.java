...
import com.baiflow.share.entity.BfShareAccessLog;
...
import com.baiflow.share.mapper.BfShareAccessLogMapper;
...
public class BfShareLinkServiceImpl extends ServiceImpl<BfShareLinkMapper, BfShareLink> implements BfShareLinkService {
    ...
    public ShareLinkInfo createShare(CreateShareRequest req, String userId) {
        ...
        BfShareLink sl = new BfShareLink();
    ...
    @Override public IPage<ShareLinkInfo> listShares(String userId, boolean isAdmin, String status, int page, int size) {
        ...
        List<BfShareLink> list; long total;
        if (isAdmin) {
            list = lambdaQuery()
                    .eq(status != null && !status.isBlank(), BfShareLink::getStatus, status)
                    .orderByDesc(BfShareLink::getCreatedAt)
            ...
            list = lambdaQuery()
                    .eq(BfShareLink::getCreatedBy, userId)
                    .eq(status != null && !status.isBlank(), BfShareLink::getStatus, status)
                    .orderByDesc(BfShareLink::getCreatedAt)
                    .last("LIMIT " + offset + ", " + size)
                    .list();
    ...
    public Map<String, Object> verifyPrivatePassword(String token, String password, HttpServletRequest request) {
        ...
        BfFileItem target = fileService.getById(sl.getTargetFileItemId());
    ...
    public IPage<FileItemInfo> browseShareFolder(String token, String parentId, int page, int size,
        ...
        if (parentId != null && !parentId.isBlank()) {
            ...
            BfFileItem parent = fileService.getById(parentId);
            if (parent == null) { throw new BusinessException(ErrorCode.NOT_FOUND, "文件夹不存在"); }
            folderId = parentId;
        }
        // 使用 file list 逻辑但不做权限校验
        BfFileItem shareTarget = fileService.getById(sl.getTargetFileItemId());
        List<BfFileItem> items = fileService.list(new LambdaQueryWrapper<BfFileItem>()
                .eq(BfFileItem::getStorageRootId, shareTarget.getStorageRootId())
                .eq(BfFileItem::getParentId, folderId)
                .eq(BfFileItem::getStatus, "ACTIVE")
                .orderByDesc(BfFileItem::getItemType)
                .orderByAsc(BfFileItem::getName));
        int total = items.size();
        int from = Math.min((page-1)*size, total);
        int to = Math.min(from+size, total);
        List<FileItemInfo> recs = (from<total ? items.subList(from,to) : List.<BfFileItem>of())
    ...
    public Resource downloadShareFile(String token, String fileId, String privacyToken, HttpServletRequest request) {
        BfShareLink sl = validateAndLog(token, "DOWNLOAD", request);
        ...
        BfFileItem file = fileService.getById(fileId);
    ...
    @Override
    public IPage<BfShareAccessLog> getShareAnalytics(String shareLinkId, int page, int size) {
        LambdaQueryWrapper<BfShareAccessLog> qw = new LambdaQueryWrapper<>();
        qw.eq(BfShareAccessLog::getShareLinkId, shareLinkId)
          .orderByDesc(BfShareAccessLog::getCreatedAt);
        return logMapper.selectPage(new Page<>(page, size), qw);
    }

    // ---- 内部辅助 ----
    private BfShareLink validateAndLog(String token, String action, HttpServletRequest request) {
        ...
        List<BfShareLink> all = lambdaQuery()
                .eq(BfShareLink::getStatus, ShareStatus.ACTIVE.name())
                .orderByDesc(BfShareLink::getCreatedAt)
                .list();
        for (BfShareLink sl : all) {
    ...
    private void incrementView(BfShareLink sl) {
        redisTemplate.opsForValue().increment(REDIS_SHARE_VIEW_KEY + sl.getId());
    }

    private void recordLog(BfShareLink sl, String action, HttpServletRequest req, boolean success, String reason) {
        BfShareAccessLog logEntry = new BfShareAccessLog();
