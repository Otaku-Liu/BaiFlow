# 12 · Android 离线模式（Offline Mode）方案

> 状态：**已实现**（2026-08-07）；后端 `updatedAfter` 增量 + Android Room/三态入口/SyncService/WorkManager，`assembleDebug` 通过
> 相关：`docs/05-android.md`、`docs/07-quick-notes.md`（Phase 3 离线同步）、`docs/09-auth-sessions.md`、`baiflow-android/app/src/main/java/com/baiflow/android/`

Android 客户端支持本地、在线、离线三种可切换的使用状态（状态模型见 §3）。基本思路是把「设置服务器」与「登录」解耦成两个独立状态：设置服务器解锁在线能力（文件中心、同步），本地能力（随手记）在三态下始终可用；离线模式是「有服务器后必须登录」的明确例外——进离线清 token，本地笔记免登录。
...
- **Android**：
  ...
  - **离线优先**：Room 作为笔记唯一数据源（分区 = 服务器地址 或 LOCAL），NotesFragment / NoteEditActivity 直接读写 Room；`SyncService`（`data/`）负责 outbox 推 + 增量拉 + 冲突标记 + 媒体上传/缓存。媒体离线新建用 `local://` 引用（`data/MediaFiles`），同步时上传并改写为服务端 URL。
...
- **边界**：
  ...
  - 本地模式笔记若在「上传前询问」选「暂不」，留在 LOCAL 分区，在线视图不显示；换回本地模式或清服务器后仍可访问（不丢数据，见本文 §4）。
