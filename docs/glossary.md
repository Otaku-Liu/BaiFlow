...
- **Auth Session（登录会话）**
  长会话 token + 服务端逐请求校验（`bf_auth_session`）的认证模型：吊销驱动，ANDROID 滑动续期（180 天不活跃兜底）/ WEB 固定 2h。见 `docs/02-database.md`「auth_session」。
- **强制下线（Force Logout）**
  吊销某设备的登录会话，对方下次请求 401 被踢回登录；入口在 Web 个人资料弹窗「登录设备」。
- **吊销驱动（Revocation-driven）**
  会话寿命不由固定到期决定，直到被主动吊销（登出 / 强制下线 / 改密码）才失效。

## B

- **BaseActivity（应用语言基类）**
  Android 公共 Activity 基类：`attachBaseContext` 应用持久化语言，`applyOverrideConfiguration` 保留 UI 模式；语言切换走「持久化 + CLEAR_TASK 重启」避免闪黑屏。全 app Activity 继承。见 `docs/05-android.md`。
- **bf_note**
...
- **Compose Cupertino（`io.github.alexzhirkevich:cupertino`）**
  Kotlin/Compose 生态的 iOS 风格组件库（CupertinoButton / SegmentedControl / Switch 等，含 `cupertino-adaptive` 按平台自动切换）。外观接近真 iOS，但**需把 app 迁到 Jetpack Compose（重写）**——BaiFlow 当前不采用，远期可选。
...
- **I18nUtil**
  服务端语言工具类（`com.baiflow.common.util.I18nUtil`）：`translate(String)` 以「中文文案即 key」在 `i18n/messages*.properties` 查词条，按请求 `Accept-Language` 返回中/英，找不到原样返回中文。全局异常处理器用它统一翻译 `BusinessException` 的 `message`。
- **错误码（ErrorCode）**
...
- **幂等守卫（Idempotent Guard）**
  Android 防重复执行的轻量手段：方法内加标志位（如 `logoutStarted`），首行 `if (flag) return; flag = true;`，重复触发直接返回。用于 `doLogout()` 兜底连点/双弹窗导致的重复执行（重复 `startActivity` + `finish` 会触发 Fragment detach 闪退）。见 `docs/05-android.md`。
- **媒体缓存（Media Cache）**
  笔记媒体本地缓存：服务端媒体缓存 `note_media_cache/`（可重下，「我的」页可手动清理 + 上限 50–2000MB 超限 LRU）；本地新建未上传的媒体 `note_media/` **永不自动清理**。见 `docs/05-android.md`。
...
- **离线模式（Offline Mode）**
  Android **仅在线模式**（已登录即在线），无主动离线看缓存；本地模式与离线模式均已移除。见 `docs/05-android.md`。
- **Outbox（本地待同步队列）**
...
- **SegmentedControl（分段控件）**
  iOS 风格的分段选择控件。BaiFlow Android 设计系统中作为自定义组件（`ui/widget/SegmentedControl`）提供。
- **styles_ios.xml**
...
- **SSE（Server-Sent Events）**
  服务端单向实时推送（`text/event-stream`）。`GET /api/events` 维护「用户 → 连接」注册表 + 心跳清理，仅推送 `NOTE_UPDATED`。
...
- **Server Settings（服务器设置）**
  Android 运行时配置服务器地址的入口（登录页地址行 + 「我的 → 通用 → 服务器」）：地址存 SharedPreferences、**不编进安装包**，未设置不能登录；未写协议时补全协议（带端口按 http、无端口按 https），保存前默认先测一次连接。**换服务器会强制清会话**（token 与增量同步游标绑定具体服务器），Room 笔记缓存按服务器分区保留，切回仍在。见 `docs/05-android.md`「登录态」。
- **Setup Token（初始化令牌）**
...
- **上传占位行（Upload Placeholder Row）**
  文件中心上传时列表顶部渲染的临时行，外观与真实文件行一致（图标 + 文件名 + 原 meta 位置换成进度条/百分比），由 `UploadService` 任务队列（多文件顺序上传）驱动；上传完成用响应 `FileItem` 原位换真后再按排序归位，失败/取消即移除并提示；纯客户端机制，不新增后端记录。见 `docs/05-android.md`。

## V

- **viewUserId**
  管理员视角切换参数：管理员传入后以指定用户身份查看其笔记/文件，非管理员忽略。

## W

- **WorkManager**
  Android 后台任务调度（本项目传输功能已用）。随手记同步用它实现「恢复联网 / 周期」触发同步。
- **文件夹大小 / 项数**
  Web 显示子项数（`childCount`），Android 长摁弹窗显示递归字节大小（`GET /api/files/{id}/size`，递归 CTE）；隐私文件夹两项均不提供。
...
- **阅读进度**：滚动 → 防抖保存 `bf_note_progress` → 换端打开自动恢复到记录位置并提示「已恢复到上次观看位置」；回顶保存 0 清除历史
